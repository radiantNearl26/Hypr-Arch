...
# or xrandr. Prefers "HDMI-A-1" if it is active; if not, uses "eDP-1".
#
# Usage:
#   record.sh fullscreen  -> Record full screen
#   record.sh section     -> Record a selected region with slurp
...
###############################################################################
# Function to detect the monitor to use
###############################################################################
detect_output() {
...
stop_recording() {
  if [ -f "$PIDFILE" ]; then
    ...
    notify-send "Recording stopped" \
                "File saved: $(basename "$FILE_RECORDED")"
  else
